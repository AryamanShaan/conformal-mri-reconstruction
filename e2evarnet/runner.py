...
from e2evarnet.data_module import DataModule
# from data.data_transforms import VarNetDataTransform
from e2evarnet.data_transforms import VarNetDataTransform
...
def cli_main(args):
    ...
    trainer = pl.Trainer(
        ...
        # NOTE(PL2.x port): renamed from replace_sampler_ddp=True in 2.0; keeps
        # Lightning auto-injecting DistributedSampler under DDP as before.
...
def build_args(cluster_launch: bool = True):
    ...
    parser.add_argument("--min_k_fraction", type=float, default=None)
    parser.add_argument("--max_k_fraction", type=float, default=None)
    ...
    # NOTE(PL2.x port): pl.Trainer.add_argparse_args was removed in Lightning
    # 2.0. The Trainer options this repo actually uses are declared explicitly
    # below with the same defaults the old set_defaults call provided.
    ...
    # NOTE(PL2.x port): the "ddp_find_unused_parameters_false" strategy string
    # was removed in 2.x; plain "ddp" now defaults to find_unused_parameters=False.
    parser.add_argument("--strategy", type=str, default="ddp")
    parser.add_argument("--precision", type=int, default=32)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--default_root_dir", type=Path, default=default_root_dir)
    parser.add_argument("--max_steps", type=int, default=210000)
